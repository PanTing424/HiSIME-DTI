...
parser.add_argument("--mode", type=str, default="inductive", choices=["inductive", "transductive"], help="Configuration mode.")
...
# Load 3D features if a matching file exists
drug_3d_features_test = None
test_3d_path = resolve_3d_feature_path(args.test_path)
if test_3d_path.exists():
    drug_3d_features_test = np.load(test_3d_path, allow_pickle=True).item()
    print(f'Loaded 3D features from: {test_3d_path}')
else:
    print(f'3D features not found, using 2D only: {test_3d_path}')

# Configurations and model setup
cfg = get_cfg_defaults()
script_dir = os.path.dirname(os.path.abspath(__file__))
cfg_path = os.path.join(script_dir, "GraphBAN_DA.yaml" if args.mode == "inductive" else "GraphBAN.yaml")
...
# Load GraphBAN model and run inference
modelG = GraphBAN(**cfg).to(device)
checkpoint = torch.load(args.trained_model, map_location=device)
if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
    checkpoint = checkpoint["model_state_dict"]
modelG.load_state_dict(checkpoint, strict=False)
modelG.eval()

pred = []
with torch.no_grad():
    for v_d, v_d_3d, sm, v_p, esm_feat, labels, drug_3d_data in test_generator:
        sm = torch.tensor(sm, dtype=torch.float32).reshape(sm.shape[0], 1, 384).to(device)
        esm_feat = torch.tensor(esm_feat, dtype=torch.float32).reshape(esm_feat.shape[0], 1, 1280).to(device)
        v_d = v_d.to(device)
        v_d_3d = v_d_3d.to(device)
        v_p = v_p.to(device)
        labels = labels.float().to(device)

        _, _, _, score = modelG(v_d, v_d_3d, sm, v_p, esm_feat, device)
        if cfg.DECODER.BINARY == 1:
            batch_pred, _ = binary_cross_entropy(score, labels)
        else:
            batch_pred, _ = cross_entropy_logits(score, labels)
        pred.extend(batch_pred.detach().cpu().tolist())
...
mkdir(os.path.dirname(save_path) or ".")
df_test.to_csv(save_path, index=False)

print(f"\nPrediction probabilities saved to: {save_path}\n")
